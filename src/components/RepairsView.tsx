...
export default function RepairsView({
    ...
    return (
      <div className="p-8 text-center bg-white border border-outline-variant rounded-md shadow-sm select-none font-sans">
        <p className="text-on-surface-variant font-medium">
          Ninguna orden cargada. Presiona "+ Nueva Reparación" en la barra
          lateral para iniciar.
        </p>
      </div>
    );
  }
  ...
  const triggerWhatsApp = () => {
    ...
    showToast(
      "WhatsApp Preparado",
      "Simulando apertura de chat con plantilla de entrega.",
      "success",
    );
    ...
    <div className="flex-1 flex flex-col gap-6 font-sans">
      <div className="select-none mb-2">
        ...
        <p className="text-[11px] font-semibold text-on-surface-variant tracking-wider uppercase mt-1 font-sans">
          Cargado actualmente: Folio #{activeRepair.id} —{" "}
          {activeRepair.clientName || "Cliente sin nombre"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              <svg
                className="w-4 h-4 fill-current select-none"
                viewBox="0 0 24 24"
              >
            ...
            <div className="flex flex-col gap-1.5 select-none font-sans">
              <label className="text-[11px] font-bold text-on-surface-variant font-sans flex justify-between">
                ...
                <span className="text-slate-400 font-semibold">
                  {activeRepair.footnote?.length || 0}/150
                </span>
              </label>
              <textarea
                maxLength={150}
            ...
            <div className="p-5 flex flex-col gap-4 font-sans text-xs">
              <div className="relative">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    // Al presionar Enter, si hay resultados, abre el primero de la lista
                    if (e.key === "Enter" && filteredRepairsList.length > 0) {
                      onSetSelectedId(filteredRepairsList[0].id);
              ...
              <div className="border border-outline-variant rounded-md overflow-hidden max-h-[250px] overflow-y-auto">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-[#ffffff] sticky top-0 border-b border-outline-variant/60 z-10 font-sans">
                    <tr>
                      <th className="p-2.5 text-[11px] font-bold text-on-surface-variant uppercase bg-slate-50">
                        Folio
                      </th>
                      <th className="p-2.5 text-[11px] font-bold text-on-surface-variant uppercase bg-slate-50">
                        Cliente
                      ...
                      <th className="p-2.5 text-[11px] font-bold text-on-surface-variant uppercase bg-slate-50 text-right">
                        Saldo Pend.
                      </th>
                      <th className="p-2.5 text-[11px] font-bold text-on-surface-variant uppercase bg-slate-50 text-center">
                        Estado
                      </th>
                    </tr>
                  </thead>
                  <tbody className="text-xs font-sans">
                    {filteredRepairsList.map((r) => (
                      ...
                      >
                        ...
                        <td className="p-2.5 text-right font-mono">
                          ${(r.totalCost - r.advancePaid).toFixed(2)}
                        </td>
                    ...
                    {filteredRepairsList.length === 0 && (
                      <tr>
                        <td
                          colSpan={5}
                          className="p-6 text-center text-slate-400 font-sans font-medium"
                        >
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-lg border border-outline-variant w-full max-w-sm shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="p-6 bg-slate-100 flex justify-center items-center h-[350px] overflow-y-auto">
              ...
              >
                <div className="text-center font-bold text-sm tracking-wide mb-1">
                  DIGICELL REPAIRS
                </div>
                ...
                <div className="mb-2.5">
                  Folio: #{activeRepair.id}
                  <br />
                  Fecha:{" "}
                  {new Date(activeRepair.createdAt).toLocaleString("es-ES", {
                    dateStyle: "short",
                    timeStyle: "short",
                  })}
                </div>
                ...
                <div className="border-t border-b border-dashed border-slate-400 py-2 mb-3 space-y-1">
                  <strong>Cliente:</strong> {activeRepair.clientName || "N/A"}
                  <br />
                  <strong>Teléfono:</strong> {activeRepair.clientPhone || "N/A"}
                  <br />
                  <strong>Equipo:</strong> {activeRepair.deviceBrand}{" "}
                  {activeRepair.deviceModel} (
                  {activeRepair.deviceColor || "Sin color"})<br />
                  <strong>IMEI/SN:</strong> {activeRepair.deviceSerial || "N/A"}
                  <br />
                  <strong>Contraseña/PIN:</strong>{" "}
                  {activeRepair.devicePassword || "N/A"}
                  <br />
                  <strong>Condición:</strong>{" "}
                  {activeRepair.receivingCondition || "N/A"}
                  <br />
                  <strong>Accesorios:</strong>{" "}
                  {activeRepair.chargerLeft ? "Cargador " : ""}{" "}
                  {activeRepair.coverLeft ? "Funda" : ""}{" "}
                  {!activeRepair.chargerLeft && !activeRepair.coverLeft
                    ? "Ninguno"
                    : ""}{" "}
                  <br />
                  <strong className="block mt-1">
                    Falla:{" "}
                    {activeRepair.problemReported ||
                ...
                <div className="text-center mt-4">
                  <div className="flex justify-center items-center h-8 gap-[1px] bg-black max-w-[120px] mx-auto select-none opacity-90">
                    {[
                      2, 1, 3, 1, 4, 2, 1, 3, 2, 4, 1, 3, 1, 1, 2, 3, 4, 1, 2,
                    ].map((num, i) => (
                      <div
                        key={i}
                        style={{ width: `${num}px` }}
            ...
            <div className="p-4 border-t border-outline-variant bg-surface-container flex justify-end gap-3 select-none">
              ...
              <button
                onClick={() => {
                  ...
                  showToast(
                    "Comprobante enviado",
                    `Simulando impresión térmica en folio #${activeRepair.id}.`,
                    "success",
                  );
                }}
