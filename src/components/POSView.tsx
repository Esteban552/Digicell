...
export default function POSView({
  ...
  const [cashMoveCounts, setCashMoveCounts] =
    useState<DenomCounts>(emptyCounts);

  // Stock validation helper
  const stockInfo = useMemo(() => {
    if (!selectedProductId)
      return { stock: Infinity, inCart: 0, available: Infinity };
  ...
  const suggestions = useMemo(() => {
    if (!itemName) return [];
    return products
      .filter((p) => p.name.toLowerCase().includes(itemName.toLowerCase()))
      .slice(0, 4);
  }, [itemName, products]);
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    if (!selectedProductId) {
      setNameError(
        "Seleccioná un producto del catálogo o la lista de sugerencias",
      );
      hasError = true;
    } else {
      setNameError("");
    ...
    if (selectedProductId && itemQty > stockInfo.available) {
      setQtyError(
        `Stock insuficiente: solo hay ${stockInfo.available} disponibles (${stockInfo.inCart} en carro de ${stockInfo.stock})`,
      );
      hasError = true;
    } else {
      setQtyError("");
    ...
    showToast(
      "Artículo agregado",
      "Se colocó en el carro de venta actual.",
      "info",
    );
  };
        ...
        if (item.id === id) {
          ...
          if (dir === "up" && item.productId) {
            ...
            if (product && nextQty > product.stock) {
              showToast(
                "Stock insuficiente",
                `Solo hay ${product.stock} disponibles de "${item.name}".`,
                "error",
              );
              nextQty = item.qty;
            }
  ...
  const handleCancelSale = async () => {
    ...
    const ok = await confirm({
      ...
      message: "¿Seguro que deseas cancelar y vaciar la venta actual?",
  ...
  const openPayModal = () => {
    if (cart.length === 0) {
      showToast(
        "Estructura vacía",
        "Agrega algún producto antes de cobrar.",
        "error",
      );
      return;
    }
  ...
  const handleProcessPay = async () => {
    ...
    if (sumPay < total) {
      showToast(
        "Pago insuficiente",
        `Faltan $${(total - sumPay).toFixed(2)} para cubrir el total.`,
        "error",
      );
      return;
    }
  ...
  const handleCashMovement = async () => {
    ...
    const parts = DENOMS.filter((d) => (cashMoveCounts[d.value] || 0) > 0)
    ...
    <>
      ...
      <div className="flex-1 flex flex-col gap-4 select-none h-full">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs font-sans text-on-surface-variant font-medium mt-1">
              Registrar cobros y movimientos de caja.
            </p>
          </div>
          ...
          <div className="flex-1 flex gap-5 min-h-0">
            ...
            <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2">
              ...
              <form
                ...
                className="bg-white border border-outline-variant rounded-md p-4 shadow-sm relative"
              >
                <div className="grid grid-cols-5 gap-2">
                  <div className="relative col-span-2">
                    ...
                    />
                    {nameError && (
                      <p className="text-[10px] font-sans text-error font-semibold mt-0.5">
                        {nameError}
                      </p>
                    )}
                      ...
                      <div className="absolute left-0 top-full mt-1 w-full bg-white border border-outline-variant rounded-md shadow-lg z-20">
                        {suggestions.map((p) => (
                          <button
                            ...
                            type="button"
                            onMouseDown={() =>
                              selectSuggestion(p.name, p.price, p.id)
                            }
                          ...
                          >
                            ...
                            <span className="font-sans text-tertiary font-bold ml-2 shrink-0">
                              ${p.price.toFixed(2)}
                            </span>
                          </button>
                        ))}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedProductId && val > stockInfo.available) {
                          setQtyError(
                            `Stock disponible: ${stockInfo.available} (${stockInfo.inCart} en carro de ${stockInfo.stock})`,
                          );
                        } else {
                          setQtyError("");
                        }
                  ...
                  <div>
                    <div
                      className={`h-10 w-full border ${priceError ? "border-error" : "border-outline"} rounded px-3 flex items-center text-sm font-sans ${selectedProductId ? "bg-surface-container-low text-on-surface font-bold" : "text-on-surface-variant"}`}
                    >
                      {selectedProductId
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {products.map((p) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            if (outOfStock) {
                              showToast(
                                "Sin stock",
                                `"${p.name}" no tiene unidades disponibles.`,
                                "error",
                              );
                              return;
                            }
                        ...
                        >
                          <h4 className="text-xs font-bold text-on-surface truncate font-sans">
                            {p.name}
                          </h4>
                          <p className="font-sans text-tertiary font-bold text-xs mt-1">
                            ${p.price.toFixed(2)}
                          </p>
                          ...
                          <p
                            className={`text-[10px] font-sans ${remaining <= 0 ? "text-error font-bold" : p.stock <= 5 ? "text-orange-500 font-semibold" : "text-on-surface-variant"}`}
                          >
                            {outOfStock
                              ? "Sin stock"
                              : remaining === p.stock
                                ? `${p.stock} en stock`
                                : `${remaining} disp. (${inCart} en carro)`}
                          </p>
                        </button>
                      );
                    })}
            ...
            <div className="w-[380px] shrink-0 bg-white border border-outline-variant rounded-md shadow-sm flex flex-col">
                ...
                {cart.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full gap-3">
                    ...
                    <p className="font-sans text-xs text-on-surface-variant text-center max-w-[240px] font-medium leading-relaxed">
                      Utiliza la barra superior para buscar productos cargados
                      en el inventario o escanea códigos.
                    </p>
                  </div>
                ) : (
                  cart.map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 py-2 px-2 rounded hover:bg-surface-container-low group"
                    >
                      <div className="flex-1 min-w-0">
                        <h4 className="font-sans text-xs font-bold text-on-surface truncate">
                          {item.name}
                        </h4>
                        <p className="font-sans text-[11px] text-on-surface-variant font-semibold">
                      ...
                      <p className="w-16 text-right font-sans text-xs font-bold text-on-surface">
              ...
              <div className="border-t border-outline-variant p-4 space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs font-sans">
                  ...
                  {discountAmtSynced > 0 && (
                    <span className="font-bold text-tertiary shrink-0">
                      -${discountAmtSynced.toFixed(2)}
                    </span>
                  )}
                </div>
        ...
        {payModalOpen && (
          ...
          >
            <div
              className="bg-white rounded-xl border border-outline-variant shadow-xl w-[560px] p-8"
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="text-xl font-sans font-bold text-on-surface">
                Procesar Pago
              </h3>
              <p className="text-sm font-sans text-on-surface-variant font-semibold mt-1.5">
                Total a cobrar:{" "}
              ...
              <div className="mt-5">
                ...
                {(() => {
                  ...
                  const { change, short } = calcChange(
                    recv +
                      Math.max(0, Number(cardLocal) || 0) +
                      Math.max(0, Number(cashUsd) || 0) * exchangeRate,
                    total,
                  );
                  return (
              ...
              <div className="flex gap-3 mt-6">
                <button
                  onClick={closePayModal}
                  disabled={isProcessing}
                  className="flex-1 h-12 border border-outline-variant rounded-md text-sm font-semibold font-sans outline-none cursor-pointer disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={isProcessing}
                  className="flex-1 h-12 bg-primary hover:bg-primary-container disabled:opacity-50 text-white rounded-md text-sm font-bold font-sans shadow-sm outline-none cursor-pointer disabled:cursor-not-allowed"
                >
                  {isProcessing ? (
        ...
        {cashModalOpen && (
          ...
          >
            <div
              className="bg-white rounded-xl border border-outline-variant shadow-xl w-[520px] p-8"
              onClick={(e) => e.stopPropagation()}
            >
              <h3 className="text-xl font-sans font-bold text-on-surface">
                Movimiento de Caja
              </h3>
              <p className="text-sm font-sans text-on-surface-variant font-semibold mt-1">
                Registrar entrada o salida de efectivo con desglose por
                denominación.
              </p>
              ...
              <div className="mt-6 space-y-5">
                ...
                <div className="border border-outline-variant rounded-md p-4">
                  <p className="text-xs font-bold font-sans text-on-surface-variant uppercase tracking-wider mb-3">
                    Desglose por denominación
                  </p>
                  <DenominationPad
                    counts={cashMoveCounts}
                    onChange={(c) => {
                      setCashMoveCounts(c);
              ...
              <div className="flex gap-3 mt-6">
                <button
                  onClick={closeCashModal}
                  disabled={isProcessing}
                  className="flex-1 h-11 border border-outline-variant rounded-md text-sm font-semibold font-sans outline-none cursor-pointer disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
...
function InputField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[11px] font-bold text-on-surface-variant uppercase tracking-wider font-sans">
        {label}
      </label>
      <input
        type="number"
